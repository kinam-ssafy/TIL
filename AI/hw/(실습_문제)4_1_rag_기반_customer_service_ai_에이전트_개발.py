# -*- coding: utf-8 -*-
"""(실습_문제)4_1_RAG_기반_Customer_Service_AI_에이전트_개발.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1Al1NwwB97xKYolzpuAXl5dOgfmkgHA_g
...
'''
tiktoken.get_encoding() 함수를 사용하여 특정 인코딩 방식의 인코더 객체를 가져옴
cl100k_base : GPT-4, GPT-3.5-turbo 등의 OpenAI 모델에서 사용하는 토크나이저
'''
...
text_splitter = RecursiveCharacterTextSplitter(
    chunk_size=150,  # 청크 최대 크기 (문자 단위)
    chunk_overlap=30, # 청크 간 겹치는 부분 (문자 단위)
    length_function=len, # 길이를 문자로 계산
    is_separator_regex=False,
)
...
# 4. invoke the chain
'''
- invoke()는 체인을 실행하는 메서드
- 빈 딕셔너리 {}를 인자로 전달하는 이유는, 현재 프롬프트에 동적으로 채워야 할 변수(placeholder)가 없기 때문
- 만약 프롬프트에 {country}와 같은 변수가 있었다면 {"country": "한국"}처럼 값을 전달
- 체인 실행 순서: prompt → llm → StrOutputParser()
    1. 프롬프트가 메시지 리스트를 생성
    2. LLM이 응답 생성
    3. StrOutputParser가 응답을 문자열로 변환
'''
...
'''
retrievr : 벡터 스토어에서 사용자 질문과 관련된 문서를 찾아주는 역할
사용자 질문: "주말에도 배송되나요?"
   → 임베딩 모델로 벡터 변환: [0.23, -0.45, 0.67, ...]
'''
...
if 'retriever' in locals():
    ...
    '''
    나중에 다른 도구들(계산기, 날씨 API 등)을 추가하려면 이 리스트에 계속 추가하면 됨
    예: `agent_tools = [shipping_policy_retriever_tool, calculator_tool, weather_tool]`
    '''
