...
for t in range(1, T+1):
    V, E = map(int, input().split())    #V, E : 0번부터 V번 까지의 노드와 E개의 간선
    edge = [list(map(int, input().split())) for _ in range(E)]
            
    
    #Kruskal
    edge.sort(key = lambda x:x[2]) # 가중치순으로 정렬
    ...
    cnt = 0
    ans = 0
    for u, v, w in edge:  #사이클 제거는 최대 E번
        if find_set(u) != find_set(v):  # 사이클이 없음
            cnt += 1
            ans += w
            union(u, v)

        if cnt == V:    #MST는 V+1 개의 정점 (0~V), V개의 간선
            break
